//
//  SBUMessageSearchModule.Header+SwiftUI.swift
//  SendbirdSwiftUI
//
...
extension SBUMessageSearchModule.Header {
    typealias ViewConverter = MessageSearchViewConverter.Header
    
    var viewConverter: ViewConverter {
        SBViewConverterSet.MessageSearch.header
    }
}
        ...
        case .leftView:
            if let leftViewConverter = self.viewConverter.leftView.entireContent,
               let hostingView = leftViewConverter(.init(parentView: self)) {
                self.leftBarButton = SwiftUIBarButtonItem(subview: hostingView)
        ...
        case .rightView:
            if let rightViewConverter = self.viewConverter.rightView.entireContent,
               let hostingView = rightViewConverter(.init(parentView: self)) {
                self.rightBarButton = SwiftUIBarButtonItem(subview: hostingView)
