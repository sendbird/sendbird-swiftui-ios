//
//  SBUOpenMutedParticipantListUserCell+SwiftUI.swift
//  SendbirdSwiftUI
//
...
extension SBUUserCell {
    typealias ViewConverterForOpenMutedParticipant = OpenMutedParticipantListViewConverter.List.TableView.RowView
    var viewConverterForOpenMutedParticipant: ViewConverterForOpenMutedParticipant {
        SBViewConverterSet.OpenMutedParticipantList.list.tableView.rowView
    }
    ...
    func applyViewConverterForOpenMutedParticipant(_ viewType: ViewConverterForOpenMutedParticipant.ViewType) -> Bool {
        ...
        switch viewType {
        case .entireContent:
            guard let viewConverter = self.viewConverterForOpenMutedParticipant.entireContent else { return false }
            let config = ViewConverterForOpenMutedParticipant.ViewConfig.init(
                user: user,
            ...
            return true
        case .profileImage:
            guard let viewConverter = self.viewConverterForOpenMutedParticipant.profileImage.entireContent else { return false }
            let config = ViewConverterForOpenMutedParticipant.ProfileImage.ViewConfig.init(
                user: user,
                isOperator: user.isOperator,
            ...
            return true
        case .userNameLabel:
            guard let viewConverter = self.viewConverterForOpenMutedParticipant.userNameLabel.entireContent else { return false }
            let config = ViewConverterForOpenMutedParticipant.UserNameLabel.ViewConfig.init(
                user: user,
        ...
        case .operatorStateView:
            guard let viewConverter = self.viewConverterForOpenMutedParticipant.operatorStateView.entireContent else { return false }
            let config = ViewConverterForOpenMutedParticipant.OperatorStateView.ViewConfig.init(
                user: user,
                isOperator: user.isOperator
            )
            guard let hostingView = viewConverter(config) else { return false }
        ...
        case .moreButton:
            guard let viewConverter = self.viewConverterForOpenMutedParticipant.moreButton.entireContent else { return false }
            let config = ViewConverterForOpenMutedParticipant.MoreButton.ViewConfig.init(
                user: user,
