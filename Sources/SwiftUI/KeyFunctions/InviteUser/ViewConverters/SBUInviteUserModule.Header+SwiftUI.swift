//
//  SBUInviteUserModule.Header+SwiftUI.swift
//  SendbirdSwiftUI
//
...
extension SBUInviteUserModule.Header {
    typealias ViewConverter = InviteUserViewConverter.Header
    
    var viewConverter: ViewConverter {
        SBViewConverterSet.InviteUser.header
    }
}
    ...
    func applyViewConverter(_ viewType: ViewConverter.ViewType) -> Bool {
        ...
        case .rightView:
            if let rightViewConverter = self.viewConverter.rightView.entireContent,
               let hostingView = rightViewConverter(
                .init(
                    parentView: self,
                    selectedUserList: Array(self.selectedUserList ?? [])
                )) {
